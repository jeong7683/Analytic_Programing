...
# Second Graph
with(consumption3, plot(DateTime, Voltage, type="n", xlab="daytime", ylab="Voltage (volts)", axes=F, frame.plot=T));
axis(1, at=c(as.numeric(min(consumption3$DateTime)), as.numeric(min(consumption3$DateTime))+86400, 
             as.numeric(min(consumption3$DateTime))+2*86400), labels=c("Thu", "Fri", "Sat"));
axis(2, yaxs="r");
with(consumption3, points(DateTime, Voltage, type="l"));
...
# Fourth Graph
with(consumption3, plot(DateTime, Global_reactive_power, type="n", xlab="daytime", ylab="Global Reactive Power (kilowatts)", axes=F, frame.plot=T));
axis(1, at=c(as.numeric(min(consumption3$DateTime)), as.numeric(min(consumption3$DateTime))+86400, 
             as.numeric(min(consumption3$DateTime))+2*86400), labels=c("Thu", "Fri", "Sat"));
axis(2, yaxs="r");
with(consumption3, points(DateTime, Global_reactive_power, type="l"));